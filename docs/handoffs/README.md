...
- **Briefs are consumed on a branch.** Under DEC-083 the session that executes a brief works on `<type>/<slug>`, opens a pull request, and stops; the brief's status flip and its report ride in that PR. A feature's own state lives in its GitHub issue and PR — a handoff carries only what no PR holds.
