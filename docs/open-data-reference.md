...
5. **Outcome, and what the subscriber sees.** Fully read with zero open → **Clean** (silence). One or more open → **Count** (digest). Everything else — **Uncertain** or **Unreachable** — sends nothing, logs at **ERROR**, and makes the rake task **exit non-zero** (DEC-071, DEC-077). Outcomes resolve and sends go out only after every plate has been fetched and the run-level check below has run (DEC-082).
6. **Run summary.** One line closing every run: outcomes, sends, duration, `rows`, `rows_with_amount_due`, `rows_without_amount_due`, `unknown_keys`. Two conditions raise it to **ERROR**: `rows > 0` with `rows_with_amount_due == 0` (the column is gone or renamed — the one false all-clear this exists to catch), and `unknown_keys` non-empty (the schema moved). Neither sends anything to a subscriber (DEC-069, DEC-073). The check runs between the fetch pass and the resolve pass: the first condition resolves every fetched plate to **Uncertain** (reason code `drift`) — no sends, no Clean in the record; the second leaves outcomes and digests untouched, because `amount_due` is still readable. Either condition fails the run — the task exits non-zero on a summary ERROR as well as on any Uncertain or Unreachable plate (DEC-082).
