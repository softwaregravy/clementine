...
**Owner:** John · **Status:** Draft for review · **Last updated:** 2026-09-20
...
- **Run outcome model — per plate, per daily run.** Four outcomes, and the boundary between the first and third is the invariant:
  ...
  - **Uncertain** — a usable response the run cannot trust: the run-level drift signal, a page of exactly 5,000 rows (truncation), a non-transient 4xx, or a 2xx body that parses but is not a JSON array → **no send; ERROR log; the run exits non-zero.**
