...
- **Work comes from the Clementine project** (https://github.com/users/softwaregravy/projects/3): each issue carries a numeric **Rank**, in gaps of ten. Take the lowest-ranked open item; insert a new task between two by giving it a rank between theirs; renumber only when a gap runs out (DEC-083).
