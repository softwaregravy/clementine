...
- **Work comes from the Clementine project** (https://github.com/users/softwaregravy/projects/3): each issue carries a numeric **Rank**, in gaps of ten. Take the lowest-ranked item not yet Done — `gh project item-list 3 --owner softwaregravy --format json --jq '[.items[] | select(.status != "Done")] | min_by(.rank) | "#\(.content.number) \(.title)"'` — read its issue and everything it links, then work it to a PR. Status moves to In Progress when a PR links the issue and to Done when it merges (built-in workflows). Insert a new task between two by giving it a rank between theirs; renumber only when a gap runs out (DEC-083).
- **Every change reaches `main` through a GitHub pull request** (DEC-083). Branch off fresh `main` as `<type>/<slug>` (`docs/`, `build/`, `feat/`, `ops/`), commit there, push the branch, open the PR with `gh pr create`, then **stop and report the PR URL** — John reviews and merges on GitHub. Never commit to `main`; never push `main`. Answer review comments with new commits on the same branch, after checking the PR is still open (`gh pr view <n> --json state`): a merged PR's branch is already deleted, and a push would resurrect it without a PR. PR body: what and why with DEC ids; the files John hand-reviews (P14); pasted verification (`bin/rspec`, `bin/rubocop`) or "docs only".
